        ...
        <div class="flex justify-center items-end gap-6 mb-10">
            ...
            <div class="flex flex-col items-center">
                @if($leaderboard[1]->profile_picture)
                    <img src="{{ asset('storage/' . $leaderboard[1]->profile_picture) }}" class="w-16 h-16 rounded-full object-cover mb-2 shadow" />
                @else
            ...
            <div class="flex flex-col items-center">
                @if($leaderboard[0]->profile_picture)
                    <img src="{{ asset('storage/' . $leaderboard[0]->profile_picture) }}" class="w-20 h-20 rounded-full object-cover mb-2 shadow-lg" />
                @else
            ...
            <div class="flex flex-col items-center">
                @if($leaderboard[2]->profile_picture)
                    <img src="{{ asset('storage/' . $leaderboard[2]->profile_picture) }}" class="w-14 h-14 rounded-full object-cover mb-2 shadow" />
                @else
            ...
            <table class="w-full min-w-90">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="{{ auth()->id() == $player->id ? 'bg-blue-50' : 'hover:bg-gray-50' }} transition">
                        <td class="py-4 px-4 font-bold text-gray-500 whitespace-nowrap">
                            @if($i === 0) 🥇
                            @elseif($i === 1) 🥈
                            @elseif($i === 2) 🥉
                            @else #{{ $i + 1 }}
                        ...
                        <td class="py-4 px-4">
                            <div class="flex items-center gap-3">
                                @if($player->profile_picture)
                                    <img src="{{ asset('storage/' . $player->profile_picture) }}" class="w-9 h-9 rounded-full object-cover shrink-0" />
                                @else
                        ...
                        <td class="py-4 px-4 text-right font-bold text-lg whitespace-nowrap {{ $player->total_points > 0 ? 'text-green-600' : 'text-gray-400' }}">
                            {{ $player->total_points }}
                        </td>
                    </tr>
                    @endforeach
