...
<style>
    ...
    .podium-title  { animation: titleDrop 0.8s cubic-bezier(0.34, 1.56, 0.64, 1) 0.3s both; }
    @keyframes titleDrop {
        from { transform: translateY(-80px) scale(0.5); opacity: 0; }
        to   { transform: translateY(0) scale(1); opacity: 1; }
    }

    .podium-stage { display: flex; align-items: flex-end; justify-content: center; gap: 8px; height: 260px; position: relative; }
    .podium-block {
        width: 120px; border-radius: 12px 12px 0 0;
        display: flex; flex-direction: column; align-items: center; justify-content: flex-end;
        padding-bottom: 12px; transform: translateY(100%); opacity: 0;
        transition: transform 0.9s cubic-bezier(0.34, 1.2, 0.64, 1), opacity 0.6s ease;
        position: relative; overflow: visible;
    }
    .podium-block.rise { transform: translateY(0); opacity: 1; }
    .podium-1st { height: 220px; background: linear-gradient(180deg, #f59e0b, #d97706); order: 2; transition-delay: 0.2s; }
    .podium-2nd { height: 160px; background: linear-gradient(180deg, #9ca3af, #6b7280); order: 1; transition-delay: 0.6s; }
    .podium-3rd { height: 120px; background: linear-gradient(180deg, #cd7c3a, #92400e); order: 3; transition-delay: 0.9s; }

    .podium-player { position: absolute; top: -90px; display: flex; flex-direction: column; align-items: center; gap: 4px; opacity: 0; transform: translateY(20px); transition: opacity 0.5s ease, transform 0.5s ease; }
    .podium-player.show { opacity: 1; transform: translateY(0); }
    .podium-avatar { width: 60px; height: 60px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; font-weight: 800; color: white; box-shadow: 0 4px 15px rgba(0,0,0,0.3); border: 3px solid white; }
    .podium-1st .podium-avatar { background: linear-gradient(135deg, #fbbf24, #f59e0b); animation: pulse-winner 1.5s ease-in-out infinite 1.5s; }
    .podium-2nd .podium-avatar { background: linear-gradient(135deg, #d1d5db, #9ca3af); }
    .podium-3rd .podium-avatar { background: linear-gradient(135deg, #d97706, #92400e); }
    @keyframes pulse-winner {
        0%, 100% { box-shadow: 0 4px 15px rgba(251,191,36,0.4); transform: scale(1); }
        50%       { box-shadow: 0 4px 30px rgba(251,191,36,0.8); transform: scale(1.08); }
    }
    .podium-name { font-size: 0.75rem; font-weight: 700; color: white; text-align: center; max-width: 110px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; text-shadow: 0 1px 4px rgba(0,0,0,0.5); }
    .podium-score-badge { font-size: 0.7rem; font-weight: 600; color: rgba(255,255,255,0.85); text-shadow: 0 1px 3px rgba(0,0,0,0.4); }
    .podium-rank-number { font-size: 1.5rem; font-weight: 900; color: rgba(255,255,255,0.9); line-height: 1; }
    .podium-medal { font-size: 2rem; line-height: 1; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.3)); }
    .podium-1st::after { content: ''; position: absolute; bottom: -10px; left: 50%; transform: translateX(-50%); width: 100px; height: 20px; background: radial-gradient(ellipse, rgba(251,191,36,0.6) 0%, transparent 70%); border-radius: 50%; filter: blur(6px); }
    #podium-close-btn { animation: fadeInUp 0.5s ease 2s both; }
    ...
    /* Phase animations */
    .phase { opacity: 0; transform: translateY(30px); transition: opacity 0.6s ease, transform 0.6s ease; }
    .phase.show { opacity: 1; transform: translateY(0); }
    ...
    #correct-answer-number.pop { transform: scale(1); }
    ...
    .wait-pulse { animation: waitPulse 2s ease-in-out infinite; }
...
<div id="results-overlay" class="{{ $initialState['status'] === 'closed' ? '' : 'hidden' }}">
    ...
    <div class="relative z-10 w-full max-w-2xl mx-auto flex flex-col items-center gap-6">
        ...
        <div id="phase-waiting" class="phase text-center pb-4">
            <p class="text-indigo-300 text-sm wait-pulse">⏳ En attente de la prochaine question par le maître du jeu...</p>
        </div>
    </div>
</div>
...
<div id="podium-overlay">
    ...
    <div class="relative z-10 flex flex-col items-center gap-8 px-4">
        ...
        <button id="podium-close-btn"
            onclick="document.getElementById('podium-overlay').classList.add('hidden')"
...
<script>
(function () {
    const CONFETTI_COLORS = ['#fbbf24','#f59e0b','#a78bfa','#60a5fa','#34d399','#f472b6','#fff','#fb923c'];
...
(function () {
    ...
    function spawnConfetti() {
        ...
        for (let i = 0; i < 100; i++) {
            ...
            el.className = 'confetti-piece';
            const color = CONFETTI_COLORS[Math.floor(Math.random() * CONFETTI_COLORS.length)];
            el.style.cssText = `left:${Math.random()*100}%;width:${6+Math.random()*8}px;height:${(6+Math.random()*8)*1.4}px;background:${color};border-radius:${Math.random()>.5?'50%':'0%'};animation-duration:${2+Math.random()*2.5}s;animation-delay:${Math.random()*3}s;`;
            container.appendChild(el);
        }
    }
    ...
    function buildLeaderboard(rows) {
        ...
        rows.forEach((r, i) => {
            ...
            div.className = `result-row flex items-center gap-3 p-3 rounded-xl border ${r.is_me ? 'is-me border-indigo-400' : 'border-white/10 bg-white/5'}`;
            ...
            div.innerHTML = `
                ...
                <div class="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold text-white border-2 border-white/30"
                     style="background: hsl(${(r.rank * 47) % 360}, 60%, 40%)">
                    ${r.initials}
                </div>
    ...
    async function poll() {
        try {
            const res = await fetch('{{ route('quiz.state') }}', { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } });
            if (!res.ok) return;
